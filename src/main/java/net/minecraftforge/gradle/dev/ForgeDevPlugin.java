...
public class ForgeDevPlugin extends DevBasePlugin {
    ...
    private void createEclipseTasks() {
        ...
        makeTask(EclipsePlugin.ECLIPSE_TASK_NAME).dependsOn("eclipseClean", "eclipseForge");
    }
    ...
    private void createMiscTasks() {
        ...
        {
            task4.getOutputs().upToDateWhen(Constants.SPEC_FALSE);
    ...
    private void createPackageTasks() {
        ...
        Zip userDev = makeTask("packageUserDev", Zip.class);
        {
            ...
            userDev.from((Callable<File>) () -> ArchiveTaskHelper.getArchivePath(patchZipFML));
            userDev.from((Callable<File>) () -> ArchiveTaskHelper.getArchivePath(patchZipForge));
            ...
            userDev.from(delayedFile(FML_VERSION), new CopyInto("src/main/resources"));
